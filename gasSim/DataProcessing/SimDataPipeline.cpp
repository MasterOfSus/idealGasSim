...
void GS::SimDataPipeline::processData(const Camera& camera, const RenderStyle& style,
                            					bool mfpMemory) {
	processing.store(true);
  ...
  while (true) {
		rawDataLock.lock();
    rawDataCv.wait_for(rawDataLock, std::chrono::milliseconds(100),
			[this] {
...
/*
void RGBA32ToRGBA8(const UInt_t* rgbaBffr, unsigned w, unsigned h, std::vector<sf::Uint8>& pixels) {
...
void RGBA32ToRGBA8(const UInt_t* rgbaBffr,
...
// read VERY carefully during reread, weird naming changes might hvae fucked up everything
void GS::SimDataPipeline::processGraphics(const std::vector<GasData>& data,
  ...
  tempRenders.reserve((data.back().getTime() - data.front().getTime()) / gDeltaTL +
                  1);
...
void GS::SimDataPipeline::processStats(const std::vector<GasData>& data, bool mfpMemory) {
...
std::vector<GS::TdStats> GS::SimDataPipeline::getStats(bool emptyQueue) {
  if (emptyQueue) {
    std::deque<TdStats> tempStats;
		std::lock_guard<std::mutex> lastStatGuard {lastStatMtx};
		// hopefully locks stats for less time by swapping, then converting to vector after releasing lock
		{ // lock scope
		std::lock_guard<std::mutex> statsGuard {statsMtx};
		lastStat = stats.back();
