...
bool GS::Gas::contains(const Particle& p) {
	if (particles.size()) {
		...
		return (
			r <= pos.x && pos.x <= boxSide - r
			&&
			r <= pos.y && pos.y <= boxSide - r
			&&
			r <= pos.z && pos.z <= boxSide - r
			&&
...
GS::Gas::Gas(std::vector<GS::Particle>&& particles, double boxSide, double time) :
	...
	// skipping particles size verification, delegating to methods to provide correct behaviour, ecc.
	if (boxSide <= 0) {
...
GS::Gas::Gas(unsigned particlesN, double temperature, double boxSide, double time) :
	boxSide(boxSide), time {time}
{
	...
	if (particlesN) {
		...
		double maxSpeed = sqrt(30./M_PI * temperature/Particle::getMass()); // should be correct expression
		...
		double missingEnergy {
			3 * particlesN * temperature -
			std::accumulate(
				particles.begin(), particles.end(),
				0.,
				[] (double& acc, const Particle& p) {
					acc += p.speed.norm()*p.speed.norm();
				}
			) * Particle::getMass() / 2.
		};
		particles.emplace_back(
			Particle({
				...
				Vector3d(
					...
					2. * missingEnergy / Particle::getMass()
				)
			})
		);
	}
...
GS::PPCollision GS::Gas::firstPPColl() {
	...
	auto getBestPPCollision {
		[] (PPCollision& c, Particle* p1, Particle* p2) {
  ...
  threads.push_back(std::thread([&]() {
    ...
    for (size_t i {0}; i < endIndex; ++i) {
      ...
      getBestPPCollision(c, particles.data() + trI.first, particles.data() + trI.second);
    }
    bestColls[0] = c;
  }));
