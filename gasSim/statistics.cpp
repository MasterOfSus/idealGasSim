...
void TdStats::addData(const Gas& gas, const Collision* collision) {
  if (gas.getParticles().size() != nParticles_) {
    ...
    if (collision->getType() == "Wall") {
      ...
      if (lastCollTimes_[gas.getPIndex(wallColl->getFirstParticle())] !=
          -INFINITY) {
        double t = lastCollTimes_[gas.getPIndex(wallColl->getFirstParticle())];
        freePaths_.emplace_back(wallColl->getFirstParticle()->speed.norm() *
                                (time_ - t));
      }
      ...
      if (lastCollTimes_[gas.getPIndex(partColl->getFirstParticle())] !=
          -INFINITY) {
        double lastCollTime{
            lastCollTimes_[gas.getPIndex(partColl->getFirstParticle())]};
        freePaths_.emplace_back(partColl->getFirstParticle()->speed.norm() *
                                (time_ - lastCollTime));
      }
      if (lastCollTimes_[gas.getPIndex(partColl->getSecondParticle())] !=
          -INFINITY) {
        double lastCollTime =
            lastCollTimes_[gas.getPIndex(partColl->getSecondParticle())];
        freePaths_.emplace_back(partColl->getSecondParticle()->speed.norm() *
                                (time_ - lastCollTime));
      }
      lastCollTimes_[gas.getPIndex(partColl->getFirstParticle())] = time_;
      lastCollTimes_[gas.getPIndex(partColl->getSecondParticle())] = time_;
    }
    std::transform(gas.getParticles().begin(), gas.getParticles().end(),
                   std::back_inserter(speeds_),
                   [](const Particle& p) { return p.speed; });
  }
}
