...
namespace GS {
	...
	class Gas {
		public:
			...
			// in a cubical lattice and uniform distribution for speed norm and direction
			Gas(unsigned particlesN, double temperature, double boxSide, double time = 0.);

			const std::vector<Particle>& getParticles() const { return particles; }
			double getBoxSide() const { return boxSide; }
			double getTime() const { return time; }

			void simulate(unsigned iterationsN, SimOutput& SimOutput);
