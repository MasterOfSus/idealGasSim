...
int main(int argc, const char* argv[]) {
    ...
    std::cout << "Simulation time: " << simTime.count()
              ...
              << std::endl;
		std::cout << "Total virtual time = " << myGas.getTime() << ", iterations number = " << iterNum << " frames per iteration = " << ((double) video.size())/iterNum << " max frames per second = " << ((double) video.size()) / simTime.count() << std::endl;
